        ...
        switch (strings.length) {
            default:
                ...
                for (int i = 0; i < explaininv.getSize(); i++)
                    explaininv.setItem(i, new ItemBuilder(Material.STAINED_GLASS_PANE).setColor(DyeColor.BLACK).setNoName().build());
                ...
                explaininv.setItem(15, new ItemBuilder(Material.MAP).setDisplayName("§b§oHow do i check my CBs?").addLoreLine("§7§o/cb status").build());
            ...
            case 1:
                if (strings[0].equalsIgnoreCase("list")) {
                    ...
                    for (CasusBelli cbobj : CasusBelli.listCBs()) {
                        if (cbobj.isPending()) {
            ...
            case 3:
                ...
                if (CasusBelli.checkIfFactionAlreadySentCB(instance.getPlayerData(player).getCurrentFactionData(), instance.getFactionManager().getFactionByName(strings[0]))) {
                ...
                CasusBelli cb = CasusBelli.createCB(instance.getPlayerData(player).getCurrentFactionData(), instance.getFactionManager().getFactionByName(strings[0]), strings[2], CBReason.getReasonByName(strings[1]));
                player.sendMessage(Message.CB_SUCCESSFULLY_SENT.getMessage());
                Bukkit.getPluginManager().callEvent(new CBRequestEvent(cb, player));
