...
public class FlagsListener implements Listener {
    ...
    public void onMonsterSpawn(EntitySpawnEvent e) {
        for (FactionData factions : instance.getFactionManager().getFactions()) {
            if (factions.getChunks().contains(instance.getChunkManager().getChunkDataByChunk(e.getLocation().getChunk())) && !factions.getAllowedFlags().contains(FactionFlag.MONSTER.getName()) && e.getEntity() instanceof Monster) {
                e.setCancelled(true);
            }
        }
    }
    ...
    public void onAnimalSpawn(EntitySpawnEvent e) {
        for (FactionData factions : instance.getFactionManager().getFactions()) {
            if (factions.getChunks().contains(instance.getChunkManager().getChunkDataByChunk(e.getLocation().getChunk())) && !factions.getAllowedFlags().contains(FactionFlag.ANIMALS.getName()) && e.getEntity() instanceof Animals) {
                e.setCancelled(true);
            }
        }
    }
